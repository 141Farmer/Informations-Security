...
def expandedKeysGenerate(masterKey, rounds):
    expandedKeys=[]
    numWords=44
    words=[None]*numWords
    for i in range(4):
...
def aesBlockEncrypt(plaintext,keys,rounds):
    blockSize=len(keys[0])
...
def aesBlockDecrypt(ciphertext,keys,rounds):
    blockSize=len(keys)

    plaintext=AddRoundKey(ciphertext,keys[-1]) 

    for i in range(rounds-1,0,-1):
        plaintext=InverseShiftRow(plaintext)
        plaintext=InverseByteSub(plaintext)
        plaintext=AddRoundKey(plaintext,keys[i])
...
def aesEncrypt(plaintext,keys,rounds,iV):
    blockSize=len(keys[0])
    ...
    for i in range(0,length,blockSize):
        ...
        plaintemptext=AddRoundKey(string,currentVector)
        ciphertemptext=aesBlockEncrypt(plaintemptext,keys,rounds)
        currentVector=ciphertemptext
        ciphertext+=ciphertemptext

    return ciphertext

def aesDecrypt(ciphertext,keys,rounds,iV):
    blockSize=len(keys[0])
    ...
    for i in range(0,length,blockSize):
        ...
        ciphertemptext=aesBlockDecrypt(string,keys,rounds)
        plaintemptext=AddRoundKey(ciphertemptext,currentVector)
...
def main():
    ...
    initialVector=Generate(block_size)
    print('Initial vector : ',initialVector)

    masterKey=Generate(block_size)
    keys=expandedKeysGenerate(masterKey,rounds)
    print('Master Key     : ',masterKey)
    
    encryptedtext=aesEncrypt(plaintext,keys,rounds,initialVector)
    encryptedtext=''.join(chr(c) for c in encryptedtext)
    print('Encrypted text : ',encryptedtext)

    decryptedtext=aesDecrypt(encryptedtext,keys,rounds,initialVector)
