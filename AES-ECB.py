...
def aesEncrypt(plaintext,keys,rounds):
    blockSize=len(keys[0])
...
def aesDecrypt(ciphertext,keys,rounds):
    blockSize=len(keys[0])
    length=len(ciphertext)
    plaintext=[]
    for i in range(0,length,blockSize):
        string=ciphertext[i:i+blockSize]
        plaintext+=aesBlockDecrypt(string,keys,rounds)
    return plaintext
...
def main():
    ...
    decryptedtext=aesDecrypt(encryptedtext,keys,rounds)
